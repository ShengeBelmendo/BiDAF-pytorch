...
class BiDAF(nn.Module):
    def __init__(self, args):
        ...
        self.att_weight_c = Linear(args.bert_layer_size, 1)
        self.att_weight_q = Linear(args.bert_layer_size, 1)
        self.att_weight_cq = Linear(args.bert_layer_size, 1)
...
#         self.att_weight_self3 = Linear(args.hidden_size * 2, 1)
        ...
        # 5. Modeling Layer
        self.modeling_LSTM1 = LSTM(input_size=args.bert_layer_size * 4,
                                   hidden_size=args.hidden_size,
                                   bidirectional=True,
                                   batch_first=True,
                                   dropout=args.dropout)
        ...
        # 6. Output Layer
        self.p1_weight = Linear(args.bert_layer_size * 4 + args.hidden_size * 2, 1, dropout=args.dropout)
        self.p2_weight = Linear(args.bert_layer_size * 4 + args.hidden_size * 2, 1, dropout=args.dropout)
...
#         q = self.context_LSTM((q, q_lens))[0]
        ...
        # 4. Attention Flow Layer
        g = att_flow_layer(c_word, q_word,c_mask,q_mask)
...
def test(bert,model,eval_examples,eval_features,args):
    ...
    for step, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
        ...
        with torch.no_grad():
            query_embeddings, _ = bert(query_input_ids, token_type_ids=None, attention_mask=query_input_mask,output_all_encoded_layers=False)
            doc_embeddings, _ = bert(doc_input_ids, token_type_ids=None, attention_mask=doc_input_mask,output_all_encoded_layers=False)
